...
import collections
from QASystem.utils.Bert_sim.bert.config import Config

from QASystem.utils.Bert_sim.bert import modeling
from QASystem.utils.Bert_sim.bert import tokenization
from QASystem.utils.Bert_sim.bert import optimization
...
class SimProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
        ...
        for i in range(100):
            file_path = os.path.join(data_dir, 'qiandao.txt')
            train_df = pd.read_csv(file_path, encoding='utf-8')
            for index, train in enumerate(train_df.values):
                guid = 'train-%d' % idx
                text_a = tokenization.convert_to_unicode(str(train[0]))
...
if __name__ == '__main__':
    ...
    if cf.do_predict:
        ...
        while True:
            l1 = input("【A】:")
            l2 = input("【B】:")
            ans = model.predict(l1, l2)
            print("【similarity】:" + str(ans[0][1]))
